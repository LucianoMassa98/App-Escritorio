...
namespace E_Shop
{
    public partial class seleccionarCobroEgreso : Form
    {
        RemitoEgreso RemitoY;
        List<Pago> listaCobros;
        ListaEgresos anterior;
        public seleccionarCobroEgreso(object x, ref Form y)
        {
            InitializeComponent();
            anterior = (ListaEgresos)y;
            anterior.Visible = false;
            RemitoY = (RemitoEgreso)x;
        ...
        {
            Cliente x = Cliente.BuscarPorNombre(RemitoY.Receptor);
            if (x != null)
            {
                label4.Text = "Saldo: $" + x.Saldo.ToString();
                label1.Text = "Total Remito: $" + RemitoY.TotalVenta().ToString();
        ...
        public void completeTextBox()
        {
            ...
            double imp = RemitoY.TotalVenta() - Pago.SumarImportes(listaCobros);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (newpago != null)
            {
                try
                {
                    ...
                    if (Math.Round(importe + Pago.SumarImportes(listaCobros)) <= Math.Round(RemitoY.TotalVenta()))
                    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (dataGridView1.RowCount>0)
            {
                RemitoVenta RemitoX = new RemitoVenta();
                ...
                RemitoX.Directo = RemitoY.Directo;
