...
namespace E_Shop
{
    public partial class ListaEgresos : Form
    {
        Usuario User;
        List<RemitoEgreso> RemitosX;
        int indice;
        public ListaEgresos(object user)
        {
            InitializeComponent();
            User = (Usuario)user;
            RemitosX = new List<RemitoEgreso>();
            indice = 0;
        ...
        public void BorraRemito()
        {
            if (RemitosX.Count() > 0)
            {
                if (RemitoEgreso.Borrar(RemitosX[indice].Codigo))
                {
                    MessageBox.Show(
                        "Remito Borrado",
                        "Exito",
        ...
        public void buscarBoletas()
        {
            ...
            string fechaB = dateTimePicker1.Value.Date.ToShortDateString();
                RemitosX = RemitoEgreso.BuscarPorFecha(fechaA, fechaB);
                if (RemitosX.Count() > 0)
                {
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            if (RemitosX.Count() > 0)
            {
                if (textBox3.Text == User.Nombre) {
                    Form k = this;
                    CargarRemitoEgreso y = new CargarRemitoEgreso(User, ref k, RemitosX[indice]);
                    y.Show();
                    y.LoadRemitoX();
                    k.Visible = false;
        ...
        private void button4_Click_1(object sender, EventArgs e)
        {
            ...
            if (RemitosX.Count > 0)
            {
                new seleccionarCobroEgreso(RemitosX[indice], ref k).Show();
            }
            else
            {
